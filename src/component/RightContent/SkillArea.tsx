    ...
    return (
        <div className="flex flex-col gap-10">
            <section className="relative pl-10 pr-10 pt-20 pb-10 rounded-[0.5rem] bg-white border border-slate-100 shadow-[0_10px_40px_rgba(0,0,0,0.03)]">
                ...
                {
                    TOTAL_SKILL.map((category, index) => (
                        <div key={category.name}>
                            <div className="space-y-6">
                                <div className="flex items-center gap-3 mb-4">
                                    <div className={`p-2.5 ${category.backgroundColor} rounded-xl ${category.textColor}`}>
                                        <Icon icon={category.iconURL} className="w-8 h-8" />
                                    </div>
                                ...
                                <div className="flex flex-wrap gap-4">
                                    {
                                        category.skillSetItem.map((item) => (
                                            <SkillBadge eachSkillSetItem={item}/>
                                        ))
                                    }
                                </div>
                            </div>
